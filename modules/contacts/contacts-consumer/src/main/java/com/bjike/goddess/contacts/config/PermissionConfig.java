...
public class PermissionConfig {
    ...
    @Autowired
    private ContactPermissionAPI cusPermissionAPI;
    @Autowired
    ...
    public void generatePerssion() throws IOException {
        ...
        //插入数据库
        List<ContactPermissionTO> listTO = new ArrayList<>();
        ...
        for (int i = 0; i < descList.size(); i++) {
            ...
            ContactPermissionTO to = new ContactPermissionTO();
            to.setIdFlag(id);
            to.setDescription(desc);
            if (ContactPermissionType.LEVEL.name().equals(type.trim())) {
                to.setType(ContactPermissionType.LEVEL);
            } else if (ContactPermissionType.MODULE.name().equals(type.trim())) {
                to.setType(ContactPermissionType.MODULE);
            } else if (ContactPermissionType.POSITION.name().equals(type.trim())) {
                to.setType(ContactPermissionType.POSITION);
            } else if (ContactPermissionType.DEPART.name().equals(type.trim())) {
                to.setType(ContactPermissionType.DEPART);
            }
            to.setOperator("");
            listTO.add(to);
