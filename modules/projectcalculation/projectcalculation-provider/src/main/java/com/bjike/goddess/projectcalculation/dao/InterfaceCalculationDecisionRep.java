package com.bjike.goddess.projectcalculation.dao;

import com.bjike.goddess.common.jpa.dao.JpaRep;
import com.bjike.goddess.projectcalculation.dto.InterfaceCalculationDecisionDTO;
import com.bjike.goddess.projectcalculation.entity.InterfaceCalculationDecision;

/**
* 界面测算决策持久化接口, 继承基类可使用ｊｐａ命名查询
* @Author:			[ wanyi ]
* @Date:			[  2017-12-09 04:23 ]
* @Description:	[ 界面测算决策持久化接口, 继承基类可使用ｊｐａ命名查询 ]
* @Version:		[ v1.0.0 ]
* @Copy:   		[ com.bjike ]
*/
public interface InterfaceCalculationDecisionRep extends JpaRep<InterfaceCalculationDecision ,InterfaceCalculationDecisionDTO> { 

 }