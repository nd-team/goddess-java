package com.bjike.goddess.marketdevelopment.config;
...
//        createDeails(lines);
        ...
        //插入数据库
        List<MarPermissionTO> listTO = new ArrayList<>();
        ...
        for (int i = 0; i < descList.size(); i++) {
            ...
            MarPermissionTO to = new MarPermissionTO();
            to.setIdFlag(id);
            to.setDescription(desc);
            if (MarPermissionType.LEVEL.name().equals(type.trim())) {
                to.setType(MarPermissionType.LEVEL);
            } else if (MarPermissionType.MODULE.name().equals(type.trim())) {
                to.setType(MarPermissionType.MODULE);
            } else if (MarPermissionType.POSITION.name().equals(type.trim())) {
                to.setType(MarPermissionType.POSITION);
            } else if (MarPermissionType.DEPART.name().equals(type.trim())) {
                to.setType(MarPermissionType.DEPART);
            }
            to.setOperator("");
            listTO.add(to);
