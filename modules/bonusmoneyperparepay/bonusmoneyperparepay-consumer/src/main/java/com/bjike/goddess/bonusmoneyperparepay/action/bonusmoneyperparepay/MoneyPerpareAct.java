...
public class MoneyPerpareAct {
    ...
    public Result guidePermission(@Validated(GuidePermissionTO.TestAdd.class) GuidePermissionTO guidePermissionTO, BindingResult bindingResult, HttpServletRequest request) throws ActException {
        try {
            ...
            Boolean isHasPermission = moneyPerpareAPI.guidePermission(guidePermissionTO);
            if (!isHasPermission) {
                //int code, String msg
    ...
    public Result proCompare(Integer years, Integer month, String[] projectGroup, HttpServletRequest request) throws ActException {
        try {
            List<PerpareBO> boList = moneyPerpareAPI.projectCompare(years, month, projectGroup);
    ...
    public Result monCompare(Integer years, Integer month, HttpServletRequest request) throws ActException {
        try {
            List<PerpareBO> boList = moneyPerpareAPI.monthCompare(years, month);
            List<PerpareVO> voList = BeanTransform.copyProperties(boList, PerpareVO.class, request);
            return ActResult.initialize(voList);
        } catch (SerException e) {
            throw new ActException(e.getMessage());
