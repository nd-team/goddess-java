...
import com.bjike.goddess.costdetail.bo.CompanyBorrowedDetailBO;
 ...
 * @Description: [ 公司借入明细业务实现 ]
 * @Version: [ v1.0.0 ]
 * @Copy: [ com.bjike ]
 */
@CacheConfig(cacheNames = "costdetailSerCache")
@Service
public class CompanyBorrowedDetailSerImpl extends ServiceImpl<CompanyBorrowedDetail, CompanyBorrowedDetailDTO> implements CompanyBorrowedDetailSer {
    @Override
    public List<String> findTypeName() throws SerException {
        List<CompanyBorrowedDetail> list = super.findAll();
        if (CollectionUtils.isEmpty(list)) {
            return Collections.emptyList();
        }
        Set<String> set = new HashSet<>();
        for (CompanyBorrowedDetail model : list) {
    ...
    public List<CompanyBorrowedDetail> findByCostId(String id) throws SerException {
        ...
        List<CompanyBorrowedDetail> list = super.findByCis(dto);
        return list;
    }
}