...
<<<<<<< HEAD
     ...
     *
     * @param request 请求对象
     * @return class DismissionEmployeeVO
     * @throws ActException
     * @version v1
     */
    @GetMapping("v1/dimissonEmployeeList")
    public Result dimissonEmployeeList(HttpServletRequest request) throws ActException {
        try {
            List<DismissionEmployeeBO> list = beforeRemoveEmployeeAPI.all();
            return ActResult.initialize(BeanTransform.copyProperties(list, DismissionEmployeeVO.class, request));
        } catch (SerException e) {
            throw new ActException(e.getMessage());
        }
    }
