...
public final class UserSession {
    ...
    private static final Map<String, CurrentUser> USER_SESSIONS = new ConcurrentHashMap<>(0);
     ...
     * @return 是否已经登录
     */
    public static void put(String token, CurrentUser currentUser) {
        if (StringUtils.isNotBlank(token)) {
            currentUser.setAccessTime(LocalDateTime.now());
            USER_SESSIONS.put(token, currentUser);
        } else {
    ...
    public static User getUser(String token) {
        if (StringUtils.isNotBlank(token)) {
            CurrentUser currentUser = USER_SESSIONS.get(token);
            if (null != currentUser) {
                currentUser.setAccessTime(LocalDateTime.now());
                return null;//currentUser.getUser();
            }
            return null;
        }
        throw TOKEN_NOT_NULL;
    ...
    public static Map.Entry<String, CurrentUser> getByUserId(String user_id) {
        if (StringUtils.isNotBlank(user_id)) {
            for (Map.Entry<String, CurrentUser> entry : USER_SESSIONS.entrySet()) {
...
//                }
    ...
    public static CurrentUser get(String token) {
        if (StringUtils.isNotBlank(token)) {
            CurrentUser subject = USER_SESSIONS.get(token);
            if (null != subject) {
...
//                }
     ...
     * @return 会话信息集合
     */
    public static Map<String, CurrentUser> sessions() {
