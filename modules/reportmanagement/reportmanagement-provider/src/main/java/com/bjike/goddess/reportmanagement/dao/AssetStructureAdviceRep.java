package com.bjike.goddess.reportmanagement.dao;

import com.bjike.goddess.common.jpa.dao.JpaRep;
import com.bjike.goddess.reportmanagement.dto.AssetStructureAdviceDTO;
import com.bjike.goddess.reportmanagement.entity.AssetStructureAdvice;

/**
* 资产结构管理建议设计持久化接口, 继承基类可使用ｊｐａ命名查询
* @Author:			[ chenjunhao ]
* @Date:			[  2017-06-30 04:38 ]
* @Description:	[ 资产结构管理建议设计持久化接口, 继承基类可使用ｊｐａ命名查询 ]
* @Version:		[ v1.0.0 ]
* @Copy:   		[ com.bjike ]
*/
public interface AssetStructureAdviceRep extends JpaRep<AssetStructureAdvice ,AssetStructureAdviceDTO> { 

 }