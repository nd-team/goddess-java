...
public class MonthTargetSerImpl extends ServiceImpl<MonthTarget, MonthTargetDTO> implements MonthTargetSer {
    ...
    private List<MonthTargetBO> transformBOList(List<MonthTarget> list) throws SerException {
        List<MonthTargetBO> bos = new ArrayList<>(0);
        for (MonthTarget entity : list)
    ...
    public MonthTargetBO save(MonthTargetTO to) throws SerException {
        MonthTarget entity = BeanTransform.copyProperties(to, MonthTarget.class);
    ...
    private void countTarget(MonthTarget entity) throws SerException {
    ...
    public MonthTargetBO update(MonthTargetTO to) throws SerException {
        MonthTarget entity = super.findById(to.getId());
        if (null == entity)
    ...
    public MonthTargetBO delete(String id) throws SerException {
        MonthTarget entity = super.findById(id);
        if (null == entity)
            throw new SerException("该数据不存在");
        super.remove(entity);
        return this.transformBO(entity);
    ...
    public List<MonthTargetBO> maps(MonthTargetDTO dto) throws SerException {
        List<MonthTarget> list = super.findByPage(dto);
        return this.transformBOList(list);
    }

    @Override
    public Long getTotal() throws SerException {
        MonthTargetDTO dto = new MonthTargetDTO();
