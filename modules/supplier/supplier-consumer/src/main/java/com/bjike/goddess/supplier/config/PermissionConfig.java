...
//        createDeails(lines);
        ...
        //插入数据库
        List<SupPermissionTO> listTO = new ArrayList<>();
        ...
        for (int i = 0; i < descList.size(); i++) {
            ...
            SupPermissionTO to = new SupPermissionTO();
            to.setIdFlag(id);
            to.setDescription(desc);
            if (SupPermissionType.LEVEL.name().equals(type.trim())) {
                to.setType(SupPermissionType.LEVEL);
            } else if (SupPermissionType.MODULE.name().equals(type.trim())) {
                to.setType(SupPermissionType.MODULE);
            } else if (SupPermissionType.POSITION.name().equals(type.trim())) {
                to.setType(SupPermissionType.POSITION);
            } else if (SupPermissionType.DEPART.name().equals(type.trim())) {
                to.setType(SupPermissionType.DEPART);
            }
            to.setOperator("");
            listTO.add(to);
